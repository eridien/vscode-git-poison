...
`#!/bin/sh
# DO NOT EDIT THIS SCRIPT.  It is generated by the vscode Git Poison extension.
set -eu
...
if [ -n "$only_in_cached" ]; then
  ...
  printf 'Poison: The Git commit was cancelled because one or more staged files contain a poison pill "%s". Hint: You removed the pill but did not stage the change. Stage the updated file(s) or use Commit All and try again.\n' "$PILL"
else
  # Pills are also visible in the working copy
  printf 'Poison: The Git commit was cancelled because one or more files contain the poison pill "%s". You can view the pills using the vscode command Git Poison: View Next Pill.\n' "$PILL"
fi

# Append block info: separator, timestamp + context, then absolute paths (one per line for staged offenders)
{
  echo "******"
  printf '%s branch=%s author=%s email=%s\n' "$(date '+%Y-%m-%d %H:%M:%S')" "$branch" "$author" "$email"
  printf '%s\n' "$matches_cached" | while IFS= read -r f; do
    [ -n "$f" ] || continue
    f_no_prefix=\${f#./}
    printf '%s/%s\n' "$top" "$f_no_prefix"
  done
} >> "$msgdir/block-info.log"
